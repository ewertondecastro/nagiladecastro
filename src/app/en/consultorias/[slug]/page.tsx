...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/en/consultorias/${params.slug}`,
      languages: {
...
export default async function ConsultoriaPageEN({ params }: PageProps) {
  const dict = await getDict("en");
  const card = dict.services.cards.find((c) => c.slug === params.slug);
  if (!card) notFound();
  return <ConsultoriaDetail dict={dict} locale="en" card={card} />;
}
